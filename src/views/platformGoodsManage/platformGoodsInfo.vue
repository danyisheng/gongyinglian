<template>
    <div class="platform-goods-detail  containerBoxInfo" :style="height">
            ...
            <el-form ref="form" :model="form" :rules="rules" label-width="140px">
                <div class="goods-content">
                    ...
                    <div class="goods-content-box goods-base">
                        <base-goods-box :form="form" :showType="form.showType" isPlatform></base-goods-box>
                    </div>
                </div>
                        ...
                        <common-row>
                            <common-col v-if="form.SaleAttrs.length > 0" :lot="8">
                                <el-form-item label="供货价：" class="red-got">
                                    <span>标题栏中输入或选择内容可以进行筛选和批量填充</span>
                                    <sale-box :dataList="dataLisParams"
                        ...
                        <common-row>
                            <common-col v-if="form.SaleAttrs.length > 0" :lot="8">
                                <el-form-item label="商品销售规格：" class="red-got">
                                    ...
                                    <sale-box :SpecificationValues="SpecificationValues"
                        ...
                        <common-row>
                            <common-col v-if="form.SaleAttrs.length === 0" :lot="8">
                                <el-form-item label="价格" class="red-got">
                                    ...
                                    <smp-price-box ref="smPrice" :cityDataDesc="cityDataDesc" v-if="form.Type=== 100"
                                                   platformType="pt"></smp-price-box>
        ...
        <div class="action-box">
            ...
            <div class="btn-box" v-if="form.showType === 'add' || form.showType === 'edit'">
                <el-button @click="saveGoods('form')" size="medium" class="saveBtn">保存</el-button>
            </div>
            <div class="btn-box" v-if="form.showType === 'updat' && Status === 2">
                <el-button @click="groundSupplyGood" size="medium" type="primary">上架</el-button>
            </div>
            <div class="btn-box" v-if="form.showType === 'updat'  && Status === 1">
                <el-button @click="underSupplyGood" size="medium" type="primary">下架</el-button>
            </div>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            commitAudit() { // 提交审核信息
                ...
                this.reviewActionData(list, this.form.status, title, '商品', 'ManageReason')
            },
            groundSupplyGood() {
            ...
            reviewActionData(list, type, actionTitle, showTitle, key) { // 操作审核流程
                let params = {
                    Ids: list,
                    Status: type
                }
            ...
            changeQuantity(val) { //设置数量
                this.$set(this.form, 'Quantity', val)
            },
            changeSelectVal(val) {
                this.itemsVal = val
            },
            changeProvince(list) { // 省份带中文
                this.cityDataDesc = list.length > 0 ? list : []
            },
            provinceChangeList(val) { // 省份code的值
                this.form.SaleRegionIds = val
            },
            getProductCategoryId() { // 导航条
            ...
            saveGoods(name) {
                ...
                let params = {
                    ...
                    SupplierRegionCode:this.form.SupplierRegionCode.length > 0 ? this.form.SupplierRegionCode[this.form.SupplierRegionCode.length - 1] : '',
            ...
            getGoodsDetail(Id, type) {
                ...
                this.$actionApi.detailApiData('/api.Erp.Product.GetProductDetail', params, this).then(data => {
                    this.$nextTick(() => {
                        ...
                        this.form.SupplierRegionCode = [data.SupplierRegionCode] //供应商区域
                        if (this.form.SaleAttrs.length === 0) {
