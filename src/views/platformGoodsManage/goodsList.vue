<template>
    <div class="platform-goods-con containerBox">
            ...
            <common-row>
                ...
                <common-col :height="50">
                    <div class="btn-box displayflex ">
                        ...
                        <province-box :citys="searchBox.SupplierRegionCode" @provinceChangeList="changeCitys"></province-box>
                    </div>
                </common-col>
                <common-col :height="50">
                    <div class="btn-box displayflex ">
                        <label for="">供应商：</label>
                        <el-input v-model="searchBox.SupplierNameOrMobile" placeholder="请输入姓名/手机号"></el-input>
                    </div>
                </common-col>
            </common-row>
            <common-row>
                ...
                <el-tab-pane label="全部" name="0">
                    <el-table
                        ...
                        <el-table-column prop="date" label="供应商">
                            <template slot-scope="scope">
                                <div class="timer-box">
                                    <p><span>供应商区域： </span>{{scope.row.SupplierRegionName}}</p>
                                    <p><span>供应商名称：</span>{{scope.row.SupplierName}}</p>
                                    <p><span>供应商手机：</span>{{scope.row.SupplierMobile}}</p>
                                </div>
                            </template>
                        </el-table-column>
...
<script>
    ...
    export default {
        ...
        components: {pageBox,provinceBox},
        methods: {
            changeCitys(list){
                this.searchBox.SupplierRegionCode = list.length > 0 ? list : []
            },
            handleClick(name) {
            ...
            getData() {
                let params = {
                    ...
                    ProductType: '2',
                    SupplierNameOrMobile:this.searchBox.SupplierNameOrMobile,
                    PurchaseType:this.searchBox.PurchaseType,
                    SupplierRegionCode:this.searchBox.SupplierRegionCode.length > 0 ? this.searchBox.SupplierRegionCode[this.searchBox.SupplierRegionCode.length - 1] : ''
                }
            ...
            goodsDetail(row, type) {
                ...
                this.$router.push({name: 'platformGoodsInfo', query: {data: qs.stringify(obj)}})
            },
            supplyAddGoods() { //新增商品
                if(this.$user.UserInfo().IsSupplierCertSuc){
                    this.$router.push({name: 'platformAddGoods'})
                }else{
                    this.$message({
                        type: 'info',
                        message: '请先通过认证'
