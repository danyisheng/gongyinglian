		...
		<div class="justify-content-between">
			<div class="goods">
				...
				<div class="goodsBox">
					...
					{{ goodsDetail.name }}
				    <!-- 平台直营对账里的省营 -->
					<img src="../../assets/image/shengy.png" v-if="goodsDetail.purchaseSupplyType == '省营' ">
					<!-- 平台直营对账里的直采 -->
					<img src="../../assets/image/zhic.png" v-if="goodsDetail.purchaseSupplyType == '直采' ">
					</div>
					<div class="goodsInfo">
						...
						<div class="info">
							...
							<div class="green">{{ goodsDetail.status == 1 ? '出售中' : '下架' }}</div>
						</div>
						<!-- 平台直营对账省营才显示的 供应商姓名 手机号 省份 -->
						<div class="info" v-if="goodsDetail.purchaseSupplyType == '省营'">
							<div class="tit">供应商姓名</div>
							<div class="">{{ goodsDetail.supplierName }}</div>
						</div>
						<div class="info" v-if="goodsDetail.purchaseSupplyType == '省营'">
							<div class="tit">供应商手机号</div>
							<div class="">{{ goodsDetail.supplierPhone }}</div>
						</div>
						<div class="info" v-if="goodsDetail.purchaseSupplyType == '省营'">
							<div class="tit">供应商省份</div>
							<div class="">{{ goodsDetail.cityName }}</div>
						</div>
					</div>
				</div>
			</div>
			...
			<common-row>
				<common-col :height="50">
					<div class="btn-box displayflex ">
						...
						<el-input v-model="searchBox.deliveryNumber" placeholder="请输入发货单号" @keyup.enter.native="search"></el-input>
					</div>
				</common-col>
				<!-- <common-col :height="50">
					<div class="btn-box displayflex resCityBox">
						<label for="">收货区域：</label>
						<cityBox allCity v-model="searchBox.cityCode" @change="changeCity"></cityBox>
					</div>
				</common-col> -->
			</common-row>
			<div class="justify-content-between">
