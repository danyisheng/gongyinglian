<template>
	<div class="purchaseList containerBox">
		...
		<div class="tabs-box">
			...
			<el-table :height="tableHeight" :data="purchaseTableData" @selection-change="handleSelectionChange" style="width: 100%" v-show="cur == '0'">
				...
				<el-table-column label="采购状态" prop="cgzt">
					<template slot-scope="scope">
						<div class="timer-box">
...
export default {
	...
	components: { pageBox, cityBox, provinceBox },
	methods: {
		// 查询
		search() {
      ...
      // console.log(arr)
      this.searchBox.cityCode = arr.join()
    },
		// 导出数据
		exportData() {
			let date = new Date();
      ...
      let excelName = '';
			if(this.cur == 0) {
				...
				let purchaseExcelHeader = ['商品信息','采购单号','发货状态','采购类型','供应商姓名','供应商电话','供应商省份','发货总数','采购运输中','已入库','不补发','取消补发','待补发','补发运输中','平台货损审核中','供应商货损审核中','采购单总金额','不补发金额','取消补发金额','赔偿金额','结算金额','创建时间','发货时间'];
				let purchaseMatchingHeader = ['goodsName','purchaseNumber','deliveryStatus','purchaseSupplyType','supplierName','supplierPhone','cityName','deliverGoodsNum','purchaseIngNum','extractGoodsNum','notReissueNumber','cancelNumber','waitReplacementNum','reissueIngNum','sdAuditNum','gylAuditNum','totalMoney','notReissueMoney','cancelMoney','paidMoney','settlementAmount','createtime','purchaseTime'];
				let purchaseList = [];
				this.selPurchaseList.map(ele => {
					...
					item.purchaseSupplyType = ele.purchaseSupplyType;
					item.cityName = ele.cityName;
				...
				let compensateExcelHeader = ['商品信息','货损单号','采购类型','供应商姓名','供应商电话','供应商省份','不补发件数','不补发赔偿金额','部分赔付件数','部分赔偿金额','创建时间'];
				let compensateMatchingHeader = ['goodsName','damageNumber','purchaseSupplyType','supplierName','supplierPhone','cityName','notReissueNumber','notReissueMoney','paidNumber','paidMoney','createtime'];
				let compensateList = [];
      ...
      if(regPos.test(this.searchBox.gysName)) {
        ...
        pagerNumber: this.pages.pageSize,
				...
				purchaseSupplyType: this.searchBox.purchaseType,
				platformSupplyRegionCode: this.searchBox.cityCode,
				supplierName: name,
				supplierPhone: phone,
