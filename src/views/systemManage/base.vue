            ...
            <el-table
                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        <span>{{scope.SupplierType === 0 ? '非供应商' : scope.SupplierType === 100 ?  '普通供应商' : scope.SupplierType === 200 ? '闪店供应商' : '直营供应商'}}</span>
                    </template>
                </el-table-column>
                <el-table-column
                        width="100"
                        label="是否认证">
                    <template slot-scope="scope">
                        <span>{{scope.IsSupplierCertSuc ? '是' : '否'}}</span>
                    </template>
                </el-table-column>
                <el-table-column
                        width="90"
                ...
                <el-table-column
                        ...
                        label="设置密码">
                    <template slot-scope="scope">
                        <span>{{scope.IsExistPwd ? '是' : '否'}}</span>
                    </template>
                </el-table-column>
                <el-table-column
                        width="130"
    ...
    export default {
        ...
        methods: {
            ...
            getData() {
                ...
                this.$https.fetchNetPost('/api.Erp.Base_Manage.Base_User.GetDataList', params).then(res => {
            ...
            delUser() {
                if (this.selectTableData.length > 0) {
                    ...
                    this.$confirm('确定要删除, 是否继续?', '删除账号', {
                        ...
                        this.loadingWait = this.showLoading('请稍候...')
                        this.$https.fetchNetPost('/api.Erp.Base_Manage.Base_User.DeleteData', params).then(res => {
                            if (res.data.ErrorCode === 200) {
                                this.loadingWait.close()
                                this.getData()
                                this.$message({
            ...
            saveModal(name) {
                this.$refs[name].validate((valid) => {
                    if (valid) {
                        ...
                        this.loadingWait = this.showLoading('请稍候...')
                        this.$https.fetchNetPost('/api.Erp.Base_Manage.Base_User.SaveData', params).then(res => {
                            if (res.data.ErrorCode === 200) {
